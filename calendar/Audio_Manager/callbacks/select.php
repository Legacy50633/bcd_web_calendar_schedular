...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($aid || $tid) {
        ...
        $sql = "UPDATE thirukural_running_status SET audio_running_status = ?, adhigaram = ?, thirukkural = ?, audio_stop_status = ?, audio_pause_status= ? WHERE id = 1";
        
        // Prepare and bind the statement
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("iiiii", $runningStatus, $aid, $tid, $stopStatus,$pauseStatus);
